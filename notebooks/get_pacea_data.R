...
npi_winter_means <- npi_monthly %>%
  ...
  filter(season_year %in% 1960:2025) %>%
...
# Compute sommer means of ONI (Apr - Sep)
mei_seasonal_means <- mei %>%
  # Keep relevant months and years
  filter(year %in% 1960:2025, month %in% 4:9) %>%
...
npgo_seasonal_means <- npgo %>%
  ...
  filter(year %in% 1959:2025, month %in% c(12, 1, 2)) %>%
...
ao_seasonal_means <- ao %>%
  # Include Dec of the previous year
  filter(year %in% 1959:2025, month %in% c(12, 1, 2, 3)) %>%
...
pdo_summer_means <- pdo %>%
  filter(year %in% 1960:2025, month %in% 5:9) %>%
  mutate(date = make_date(year, month, 1),
         season_year = year) %>%
  group_by(season_year) %>%
  summarise(mean_value = mean(anomaly, na.rm = TRUE)) %>%
